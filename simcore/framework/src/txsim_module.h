 ...
 * \f$ dt_{\mathrm{simulation}} \f$往往不等于\f$ dt_{\mathrm{physical}}
 * \f$，而场景中其他物体的状态变化均由仿真时间计算得出。 任何一个场景的起始仿真时间总是从0s时刻开始。
 *
 * - [1] 前提是算法中不包含任何随机因素。
 *
...
class SimModule {
 public:
   ...
   * 中关于外部授时的说明。另外还可以通过helper对象获取订阅的消息内容，发布消息，停止当前场景等，详见
   * tx_sim::StepHelper。 该方法只有在所有模块的Reset方法成功返回后才会被调用。
   *
   * @param[in,out] helper
   * 由仿真系统负责在调用该方法前进行相应初始化并传入的助手类型，提供模块运行场景时所需要的一些API。
   *
   ...
   *
   * @param[out] helper
   * 由仿真系统负责在调用该方法前进行相应初始化并传入的助手类型，提供模块在场景结束时所需要的一些API。
   */
